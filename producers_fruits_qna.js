...
// --- CONFIGURATION ---
const CONFIG = {
    network_share_path: "/Users/Shared/CrawledData", // Ensure this path points to your data on producers
...
const openApiSpec = {
    openapi: "3.0.3",
    info: {
        title: "Fruit and Vegetable Producer Q&A API",
        version: "8.0.0",
        description: "A simplified API without caching for improved reliability, providing information on fruit and vegetable producers."
    },
    servers: [{ url: `http://localhost:${PORT}` }],
    paths: {
        "/process-single-question": {
            post: {
                summary: "Process a single question about fruit and vegetable producers",
                description: "Submits one question and returns a precise answer by reading markdown content in real-time about fruit and vegetable producers.",
                requestBody: {
                    required: true,
                    content: {
                        "application/json": {
                            schema: {
                                type: "object",
                                properties: {
                                    folder_name: { type: "string", example: "producers_data_europe" },
                                    single_question: { type: "string", example: "Who are the main organic apple producers in France?" }
                                },
        ...
        "/process-multiple-questions": {
            post: {
                summary: "Process multiple questions about fruit and vegetable producers",
                description: "Submits multiple questions (up to 80) and returns precise answers by reading markdown content in real-time about fruit and vegetable producers.",
                requestBody: {
                    required: true,
                    content: {
                        "application/json": {
                            schema: {
                                ...
                                properties: {
                                    ...
                                    questions: {
                                        ...
                                        items: { type: "string" },
                                        maxItems: 80,
                                        example: ["Who are the main organic apple producers in France?", "What varieties of tomatoes are grown in Italy?", "Are there any large-scale vegetable farms in Spain?"]
                                    }
                                },
...
function scoreChunk(chunk, question) {
    ...
    // Adjusted keyword weights for fruit and vegetable producers
...
async function readMarkdownFiles(dir, question = '') {
    ...
    const questionWords = question.toLowerCase().split(/\s+/).filter(w => w.length > 3);
    // Updated base keywords for fruit and vegetable producers
    const baseKeywords = ['fruit', 'vegetable', 'producer', 'farm', 'agriculture', 'organic', 'harvest', 'cultivation', 'varieties', 'distribution'];
...
async function getAnswerFromOllama(question, context) {
    ...
    if (chunks.length === 0) {
        ...
        return {
            answer: "Insufficient information in the provided context.",
            found: false
        };
    }
    ...
    for (const { chunk, score } of scoredChunks.slice(0, 2)) { // Process only top 2 chunks
        ...
        // Modified prompt for fruit and vegetable producers
        const prompt = `You are an expert assistant on fruit and vegetable producers. Use ONLY the CONTEXT below to answer the QUESTION. Provide a clear, concise, and precise answer in less than ${CONFIG.max_response_length} characters. If the information is insufficient, state "Insufficient information in the provided context." Today's date is ${today}.\n\nCONTEXT:\n${chunk}\n\nQUESTION:\n${question}\n\nANSWER:`;
        ...
        try {
            ...
            let answer = result.response ? result.response.trim() : "Insufficient information in the provided context.";
            console.log(`Model response: ${answer.substring(0, 100)}...`);

            const isRelevantAnswer = !answer.includes("Insufficient information") && answer.length > 20;
    ...
    const finalAnswer = bestAnswer || "Insufficient information in the provided context.";
    console.log(` ${finalAnswer.substring(0, 100)}...`);
    return {
        answer: finalAnswer,
...
function calculateRelevanceScore(answer, question) {
    ...
    const isGeneric = answer.includes("Insufficient information") || answer.length < 20;
    ...
    try {
        ...
        const processPromises = sanitizedQuestions.map(async (question, index) => {
            ...
            try {
                ...
                return {
                    question: question,
                    reponse: `Error during processing: ${error.message}`,
                    found: false,
                    processing_time_ms: processingTime
                };
            }
...
if (CONFIG.use_clustering && cluster.isMaster) {
    ...
    app.listen(PORT, () => {
        console.log(`🚀 Fruit and Vegetable Producer Q&A Server running on http://localhost:${PORT}`);
